...
# Organization
organization_info = df['Organization_name'].unique()
organization_info = organization_info.astype('str')
organization_info = organization_info.tolist()
organization_info.sort()
organization_info.insert(0, "All") #add All
...
if submit_checkbox:
    if geo_area_selectbox != "All":
        df = df[df['Organization_name'] == geo_area_selectbox]
    else:
        df = df[df['Organization_name'] == geo_area_selectbox]
    organization_description_info = df['Organization_name'].unique() #generate text
    organization_info = pd.DataFrame(organization_info)
    os.remove("input.txt")
    organization_info.reset_index(drop=True, inplace=True)
    organization_info.to_csv("input.txt")
else:
    df = df_backup
...
# Metrics logic
#================
df_metrics_geo_agg = df.groupby(['Recipient_org_city']).agg(Total_Amount_Awarded = 
                                                                       ('Amount_awarded', 'sum')).reset_index()
df_metrics_geo_agg.columns = ['City','Total Amount Awarded']
geo_count = df_metrics_geo_agg['City'].count()
...
metric_col1.metric("# of Cities", str(geo_count))
...
#================
text_analysis_column1, text_analysis_column2 = st.columns(2)
text_analysis_column1.header("Sentiment analysis")
text_analysis_column2.header("Topic Modeling")
# if submit_checkbox:
