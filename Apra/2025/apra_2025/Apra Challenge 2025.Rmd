...
reference_date <- ymd('2015-01-01') # Keeping user's specified date
    ...
    #       modes <- names(sort(table(CRM_INTERACTION_TYPE), decreasing = TRUE))[1]
    #       ifelse(length(modes) > 0, modes, 'Unknown')
    #     },
    #     .groups = 'drop'
    #   )
    # 
    # crm_agg <- crm_agg %>%
    #   left_join(crm_type_mode, by = "CONSTITUENT_ID") %>%
...
```{r}
division_temp <- divisions %>%
  group_by(segment,segment_value) %>%
  select(segment,segment_value)
```
...
```{r}
df <- rbind(df, c(0.0000000000,0.75,0.003154574,0.00,2))
