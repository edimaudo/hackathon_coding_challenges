...
frequency_info <- c(7,30, 12, 52, 365)
...
forecast_df <- function (ts_df,aggregateInput,frequencyInput,dataType) {
  if(aggregateInput == 'daily'){
    gift_data <- apply.daily(ts_df,colMeans)
    ...
    gift_data <- apply.weekly(ts_df, colMeans) 
    gift_end <- floor(0.8*length(gift_data)) 
    gift_train <- gift_data[1:gift_end,] 
    gift_test <- gift_data[(gift_end+1):length(gift_data),]
    gift_start <- c(year (start(gift_train)), month(start(gift_train)),
    ...
    gift_data <- apply.monthly(ts_df, colMeans) 
    gift_end <- floor(0.8*length(gift_data)) 
    gift_train <- gift_data[1:gift_end,] 
    gift_test <- gift_data[(gift_end+1):length(gift_data),]
    gift_start <- c(year (start(gift_train)), month(start(gift_train)))
...
```{r}
forecast.horizon <- 52
train <- forecast_df(gift_monthly,'monthly',24,"train")
```
...
```{r}
      ...
      train <- forecast_df(gift_monthly,'monthly',24,"train")
      test <- forecast_df(gift_monthly,'monthly',24,"test")
...
```{r}
start_date <- as.Date(today() + 1) 
end_date <- as.Date(today() + 32) 
date_range <- data.frame(seq(start_date, end_date,"days")) 
```

```{r}
# 30 day forecast
forecast(gift_test_tbat_forecast,h=30)
```
